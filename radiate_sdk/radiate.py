...
try:
    ...
    from radiate_sdk.utils.calibration import Calibration 
...
class Sequence:
    ...
    def __init__(self, sequence_path, config_file='radiate_sdk/config/config.yaml'):
        """
    ...
    def get_from_timestamp(self, t, get_sensors=True, get_annotations=True):
        ...
        if (get_annotations):
            ...
            if (self.annotations != None):
                ...
                if self.config['use_camera_right_rect'] or self.config['use_camera_right_raw']:
                    annotations['lidar_bev_image'] = self.get_lidar_annotations(
                        id_radar, self.config['interpolate_bboxes'], t, ts_radar, t2)
                    bboxes_3d = self.project_bboxes_to_camera(annotations['lidar_bev_image'],
    ...
    def get_id(self, t, all_timestamps, time_offset=0.0):
        ...
        ind = np.argmin(np.abs([element - t + time_offset for element in all_timestamps['time']]))
